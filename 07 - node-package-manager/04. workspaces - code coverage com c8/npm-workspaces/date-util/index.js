import StringUtil from '@nathan/string-util'
...
const yymmdd = /(?<year>\d{4}).(?<month>\d{2}).(?<day>\d{2})/g; // gm;
const ddmmyy = /(?<day>\d{2}).(?<month>\d{2}).(?<year>\d{4})/g; // gm;
